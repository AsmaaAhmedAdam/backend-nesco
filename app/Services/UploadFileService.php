...
function uploadVideo($files, $url = 'videos', $key = 'video', $model = null)
{
    $dist = storage_path('app/public/' . $url);
    if ($url != 'images' && !File::isDirectory(storage_path('app/public/videos/' . $url . "/"))) {
        File::makeDirectory(storage_path('app/public' . DIRECTORY_SEPARATOR . 'videos' . DIRECTORY_SEPARATOR . $url . DIRECTORY_SEPARATOR), 0777, true);
        $dist = storage_path('app/public' . DIRECTORY_SEPARATOR . 'videos' . DIRECTORY_SEPARATOR . $url . DIRECTORY_SEPARATOR);
    } elseif (File::isDirectory(storage_path('app/public/videos/' . $url . "/"))) {
        $dist = storage_path('app/public/videos/' . $url . "/");
    }
    $file = '';
    ...
    if (gettype($files) == 'array') {
        $file = [];
        foreach ($files as $new_file) {
            $file_name = time() . "___video_" . $new_file->getClientOriginalName();
            if ($new_file->move($dist, $file_name)) {
                $file[][$key] = $file_name;
            }
        }
        ...
        $file = $files;
        $file_name = time() . "___video_" . $file->getClientOriginalName();
        if ($file->move($dist, $file_name)) {
            $file = $file_name;
        }
    }
