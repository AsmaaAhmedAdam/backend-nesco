...
@section('main_title') {{ trans('home.categories') }}   @endsection

@section('top_title') {{ trans('home.categories') }}   @endsection


@section('sub_title')

    <a href="{{ url('admin_panel/categories/create') }}" class="btn btn-light-warning font-weight-bolder btn-sm">
        {{ trans('home.add_cat') }}
    </a>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="{{ url('admin_panel/categories/popularity'). '/' . $value->id }}" class='{{ $value->popularity == 1 ? 'is_popularity' : '' }}' title=''>
                                <i class="fa fa-star"></i>
                            </a>

                            <a href="{{ url('admin_panel/categories/'). '/' . $value->id . '/edit'}}" class='m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill' title='Edit'>
        ...
        $(document).ready(function () {
            ...
            $('#m_table_1').on('click', '.DeletingModal', function () {
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            window.location.href = '{{ url('admin_panel/categories/destroy') }}' + '/' + ID;
