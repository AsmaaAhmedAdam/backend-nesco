...
@section('main_title') {{ trans('home.edit-coupon') }}  @endsection
   ...
   <div class="col-md-12">
      ...
      <div class="card card-custom">
         ...
         <div class="card-header">
            <h3 class="card-title">{{ trans('home.edit-coupon') }}</h3>
         </div>
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6 col-sm-6 {{ $errors->has('title') ? ' has-error' : '' }}">
                        ...
                        <input type="text" name="title" required class="form-control m-input" value="{{ $Item->title }}" placeholder=" {{ trans('home.name') }} ">
                        @if ($errors->has('title'))
                     ...
                     <div class="col-lg-6 col-sm-6 {{ $errors->has('value_type') ? ' has-error' : '' }}">
                        ...
                        <select name="value_type" id="value_type" required class="form-control">
                            ...
                            <option value="value" @if($Item->value_type == 'value') {{ 'selected' }} @endif> {{ trans('home.value') }}  ( {{ trans('home.aed') }} ) </option>
                            <option value="percentage" @if($Item->value_type == 'percentage') {{ 'selected' }} @endif> {{ trans('home.percentage') }} ( % )  </option>
                        </select>
                     ...
                     <div class="col-lg-12 col-sm-12 {{ $errors->has('value') ? ' has-error' : '' }}">
                        ...
                        <input type="text" name="value" required class="form-control m-input"   value="{{ $Item->value }}" placeholder=" {{ trans('home.value') }} ">
                        @if ($errors->has('value'))
                            <span class="help-block" style="color:red">
                     ...
                     <div class="col-lg-12 col-sm-12 {{ $errors->has('value') ? ' has-error' : '' }}">
                        <input id="end_date" type="checkbox" value="1" name="date_type" @if($Item->date_type == 1) checked @endif  style="cursor: pointer">
                        <label for="end_date" style="cursor: pointer">  {{ trans('home.have-end-date') }} </label>

                     </div>


                     <div class="col-lg-12 col-sm-12 date @if($Item->date_type == 1) {{'active'}} @else {{'un_active'}} @endif {{ $errors->has('date') ? ' has-error' : '' }}" style="margin-bottom: 20px;">
                        <label> {{ trans('home.end-date') }} </label>
                        <label> </label>
                        <input type="text" id="m_datepicker_1"  name="date"  class="form-control m-input" value="{{ $Item->date }}"  readonly placeholder=" {{ trans('home.end-date') }}   .....  " />
                        @if($errors->has('date'))
                        <span class="help-block" style="color: red;">
