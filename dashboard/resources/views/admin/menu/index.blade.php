...
@section('main_title') {{ trans('home.menu') }} @endsection

@section('top_title') {{ trans('home.menu') }} @endsection

@section('sub_title')

    <a href="{{ url('admin_panel/menu/create') }}" class="btn btn-light-warning font-weight-bolder btn-sm">
        {{ trans('home.add_menu') }}
    </a>
            ...
            <table class="table table-bordered table-checkable" id="m_table_1">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($value->category)
                                <a href="{{ url("admin_panel/menu_categories/".$value->category->id."/edit") }}">
                        ...
                        <td>
                            ...
                            <a href="{{ url('admin_panel/menu/popularity'). '/' . $value->id }}" class='{{ $value->popularity == 1 ? 'is_popularity' : '' }}' title=''>
                                <i class="fa fa-star"></i>
                            </a>

                            <a href="{{ url('admin_panel/menu/'). '/' . $value->id . '/edit'}}" class='m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill' title='Edit'>
        ...
        $(document).ready(function () {
            ...
            $('#m_table_1').on('click', '.DeletingModal', function () {
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            window.location.href = '{{ url('admin_panel/menu/destroy') }}' + '/' + ID;
