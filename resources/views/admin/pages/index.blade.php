...
@section('main_title') {{ trans('home.pages') }} @endsection

@section('top_title') {{ trans('home.pages') }} @endsection

@section('sub_title')

    <a href="{{ url('admin_panel/site_pages/create') }}" class="btn btn-light-warning font-weight-bolder btn-sm">
        {{ trans('home.add_page') }}
    </a> 
            ...
            <table class="table table-bordered table-checkable" id="m_table_1">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if(!empty($images[$value->id]))
                                @foreach ($images[$value->id] as $image)
                                    <img src="{{ Custom_Image_Path('images',$image) }}?{{rand()}}" style="height:150px;width:150px;display: block;margin: auto;">
                                @endforeach
                            @endif
                        </td>
                        ...
                        {{-- <td>
                            @if($value->category)
                                <a href="{{ url("admin_panel/categories/".$value->category->id."/edit") }}">
                                    {{ $value->category->{$lang.'_title'} }}
                                </a>
                            @endif
                        </td> --}}

                        <td> {{ $value->{'main_title_'.$lang} }} </td>
                        ...
                        <td>
                            ...
                            <a href="{{ url('admin_panel/site_pages/'). '/' . $value->id . '/edit'}}" class='m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill' title='Edit'>
